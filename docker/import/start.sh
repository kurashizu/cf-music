...
# here, unlike community reference images built for arbitrary hosts. There's
# also no systemd running as PID 1 inside the container (GitHub's own
# keep-alive process is), so the cloudflare-warp package's postinst
# `systemctl enable/start warp-svc.service` call silently no-ops instead of
# actually starting a competing warp-svc instance — nothing here needs to
# guard against that the way a bare-runner setup would.
#
...
# Every job starts in a freshly pulled container with no state from any
# prior run, so without this, `registration new` below would run on every
# single job — a fresh anonymous WARP identity per run. Restoring a
# previously saved one from MinIO (see warp_identity.py) means registration
# only actually happens once, not once per job; GitHub-hosted runners share
# IP ranges across unrelated jobs, so minimizing how often we hit the
# registration endpoint at all is worth doing even though it isn't
# currently rate-limited. Must happen before warp-svc starts below: it
# loads/watches this directory on startup, so writing into it afterward
# isn't guaranteed to be picked up.
#
