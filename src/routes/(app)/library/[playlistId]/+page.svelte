<script lang="ts">
	...
	/**
	 * The whole playlist in playback order, fetched as bare tracks.
	 *
	 * In the stored order this comes from the queue endpoint, which skips
	 * the cover presigning the row endpoint does per song — on a
	 * thousand-song playlist that was the difference between one small
	 * request and ten paged ones carrying a thousand signed URLs the queue
	 * never reads.
	 *
	 * Once the view is filtered or sorted, the order on screen is the
	 * client's own and no endpoint knows it, so that path still needs every
	 * row loaded. The effect above has already started that load for
	 * exactly those views.
	 */
	async function queueTracksInViewOrder() {
		if (isCustomUnfilteredView) {
			const response = await fetch(`/api/playlists/${data.playlist.id}/queue`);
			if (response.ok) return (await response.json()) as ReturnType<typeof toQueueTracks>;
			// Fall through to the loaded rows rather than failing to play.
		}
		await list.loadAll();
	...
	async function playAll(shuffle = false) {
		...
		await player.playQueue(await queueTracksInViewOrder(), 0, shuffle);
	}
	...
	async function playFrom(index: number) {
		...
		// The queue is the whole playlist, so the starting position is
		// resolved against that rather than the rendered window.
		const tracks = await queueTracksInViewOrder();
		const queueIndex = tracks.findIndex((candidate) => candidate.videoId === song.videoId);
		await player.playQueue(tracks, Math.max(0, queueIndex));
	}
	...
	/**
	 * Creates a playlist and immediately copies into it.
	 *
	 * Without this the dialog can be a dead end: the destination list is
	 * every *other* playlist, so someone whose only playlist is the one
	 * they're standing in opens it to find nothing to pick and no way
	 * forward.
	 */
	async function createAndCopyInto() {
	...
	function rowActions(index: number, song: Song): SongRowActions {
		return {
			...
			onMenuOpenChange: (open) => {
				...
				// complete by the time either is chosen. Gated on the menu
				// actually offering them: on a playlist that can't be
				// reordered this was loading every song the moment a menu
				// opened, for actions that aren't there.
				if (open && isCustomUnfilteredView && !isAutoGenerated && !isReadOnly) {
					list.loadAll();
				}
			},
	...
	<Dialog.Content class="sm:max-w-sm">
		...
		{:else}
			<p class="text-muted-foreground text-xs">
				You don't have another playlist yet. Name one and it'll be created with these songs in it.
			</p>
		{/if}
		<div class="flex gap-2">
			<Input
				...
				disabled={copySubmitting || creatingPlaylist}
				onkeydown={(event) => event.key === 'Enter' && createAndCopyInto()}
			/>
			<Button
				variant="outline"
				class="shrink-0 gap-1.5"
				disabled={copySubmitting || creatingPlaylist || newPlaylistName.trim().length === 0}
				onclick={createAndCopyInto}
			>
				{#if creatingPlaylist}
					<LoaderCircleIcon class="size-3.5 animate-spin" />
				{:else}
