...
export const load: PageServerLoad = async ({ platform, locals }) => {
	...
	// The 3 play-history playlists share a fixed, known set of names — every
	// other auto_generated playlist is an artist grouping, named after the
	// artist itself. That's the only thing distinguishing the two smart
	// sections shown on this page; there's no separate `kind` value per
	// sub-category since both are equally read-only/CI-generated.
	...
	return {
		...
		artistPlaylists: autoGenerated.filter((p) => !playHistoryNames.has(p.name)),
		recommendedPlaylists: autoGenerated.filter((p) => playHistoryNames.has(p.name)),
		defaultPlaylistId,
